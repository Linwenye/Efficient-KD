...
def get_corpus_iterator(file_path, batch_size, is_shuffle, label_vocab=None, word_vocab=None):
    ...
    if word_vocab is not None:
        iterative_support(word_vocab.add, [[w for w in seq] for seq, _ in data])
...
def bert_data_to_tensor(batch_data, token_vocab, label_vocab, is_train):
    ...
    if is_train:
        ...
        results = {'var_seq': var_text,
                   'var_mask': var_mask,
                   'var_pos': var_position,
                   'var_len': var_len,
...
def lstm_data_to_tensor(batch_data, token_vocab, label_vocab, is_train):
    seq_lens = [len(u) + 1 for u, _ in batch_data]
    max_len = max(seq_lens)

    pad_seq = [[token_vocab.SOS_SIGN] + u + [token_vocab.PAD_SIGN] * (max_len - len(u)) for u, _ in batch_data]
    var_text = torch.LongTensor(iterative_support(token_vocab.index, pad_seq))
    var_len = torch.LongTensor(seq_lens)
    ...
    if is_train:
        ...
        for i in range(0, len(batch_data)):
            annotation = [(0, 0, "O")] + [(i + 1, j + 1, l) for i, j, l in batch_data[i][1]]
            sign_mat = [[[1] * len(label_vocab) for _ in range(max_len)] for _ in range(max_len)]
        ...
        var_label = torch.LongTensor(serial_labels) == 1
        if torch.cuda.is_available():
            var_label = var_label.cuda()

        results = {'var_seq': var_text, 'var_len': var_len, 'var_label': var_label}
    else:
        results = {'var_seq': var_text, 'var_len': var_len, 'label_vocab': label_vocab}
    return results
...
class Procedure(object):
    ...
    @staticmethod
    def evaluation(model, data_iter, token_vocab, label_vocab, source_dir, use_bert):
        model.eval()
        ...
        for batch in tqdm(data_iter, ncols=70):
            if use_bert:
                info = bert_data_to_tensor(batch, token_vocab, label_vocab, False)
            else:
                info = lstm_data_to_tensor(batch, token_vocab, label_vocab, False)

            with torch.no_grad():
                segments, _ = model.inference(**info)
            ...
            if batch == 0:
                ...
                print('oracle',oracle_tags)
        script_path = os.path.join(source_dir, "conlleval.pl")
        f1_score, msg = CoNLL_f1(text_list, predicted_tags, oracle_tags, script_path)
        return f1_score, msg, time.time() - time_start
